...
clear
url_izin="https://raw.githubusercontent.com/Sulastriii/mix/main/izin-ip-sandz"
...
REPO="https://raw.githubusercontent.com/Pelekpak/ki/main/"
REPO2="https://raw.githubusercontent.com/Sulastriii/mix/main/"
start=$(date +%s)
secs_to_human() {
...
clear
wget -q -O cf.sh "${REPO2}Fls/cf.sh"
else
clear
...
# // Ambil Config Server
wget -q -O /etc/xray/config.json "${REPO2}Cfg/config.json"
domain=$(cat /etc/xray/domain)
...
printf "q\n" | rclone config
wget -q -O /root/.config/rclone/rclone.conf "${REPO2}Cfg/rclone.conf"
cd /bin
...
wget -O /etc/banner.txt "${REPO2}Bnr/banner.txt"
...
mkdir -p /cache
cd /cache
wget -q -O menu.zip "${REPO2}Cdy/menu.zip"
7z x -pBuluJembut567 menu.zip >/dev/null 2>&1
chmod +x menu/*
