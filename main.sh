...
TIMES="10"
CHATID="5869125156"
KEY="7541590621:AAHbA_TIxR5_-tCdnolFh62TQZNUm7kr5S8"
...
echo -e " Recode By HENDRA VPN STORE${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
#Repo git
repo="https://raw.githubusercontent.com/Sulastriii/";
#########################
# USERNAME
rm -f /usr/bin/user
username=$(curl ${repo}mix/main/izin-ip-sandz | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl ${repo}mix/main/izin-ip-sandz | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl ${repo}mix/main/izin-ip-sandz | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    instalasi="https://raw.githubusercontent.com/Sulastriii/mix/main/"
...
function versionSC(){
    ...
    wget -O /root/versi/version "${instalasi}versi" >/dev/null 2>&1
}
...
<code>────────────────────</code>
<b> HENDRA STORE SCRIPT  </b>
<code>────────────────────</code>
<i>Automatic Notifications From Github Instalation</i>
"'&reply_markup={"inline_keyboard":[[{"text":"Register IP","url":"https://wa.me/6282184684197"}]]}' 
...
#cd /usr/local/sbin/
#sed -i 's/\r//' quota
wget https://raw.githubusercontent.com/Suhenn/Sipa/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
...
print_install "Memasang modul SlowDNS Server"
    wget -q -O /tmp/nameserver "${instalasi}Fls/nameserver" >/dev/null 2>&1
...
# Ganti Banner
wget -O /etc/kyt.txt "${instalasi}Bnr/issue.net"
...
wget -O /usr/sbin/ftvpn "${instalasi}Fls/ftvpn" >/dev/null 2>&1
